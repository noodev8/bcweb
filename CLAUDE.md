...
**Amazon Pricing** (a SKU-grain mirror of the Shopify flow — segment picker → WINNERS|LOSERS|ALL lists → per-SKU drill; see `docs/amz-pricing-spec.md`): `GET /amz-segments` · `GET /amz-winners?segment&days?&limit?` · `GET /amz-losers?segment&days?&limit?&coverWeeks?` · `GET /amz-all?segment` · `GET /amz-drill?code` · `GET /amz-history?code&limit?` · `GET /amz-sales?code&limit?` · `GET /amz-find?term` · `POST /amz-apply {code,newPrice,note?}` (W-A1). Key differences from Shopify: **SKU-grain** (each size = its own Amazon price/row; `code` not `groupid`); reads `amzfeed` (FBA-only, **READ ONLY** — refreshed nightly from Amazon, never written) for live price/stock, `sales` where `channel='AMZ'` for velocity; **no review/park** concept; **no live push** — W-A1 only logs to `amz_price_log` (blocks `< cost+fbafee` floor, flags `> rrp`), and the price reaches Amazon via a **client-built one-file Seller Central upload** (session basket in `src/contexts/AmzBasketContext.tsx`, no `amzfeed`/DB write). Winners = in-stock, sold in 30d; Losers = DEAD (no AMZ sale in 14d) or SLOW (cover ≥ 16wk). Amazon "margin" is **net** (price − cost − FBA fee).
