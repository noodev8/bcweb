...
Work always starts from a **group** — a **segment**, or (Shopify only, 2026-09-23) a **Google campaign** bucket (`skusummary.googlecampaign`; the Segments screen's Segment | Campaign switch, `pause`/blank hidden; `utils/pricingGroup.js`), or a **Status** (Shopify AND Amazon, 2026-09-24 — the stored `skusummary.portfolio_status` WINNERS | STEADY | NEW | HARVEST | LOSERS, set by "Update now" on the Repricing Status tab, rules in `utils/portfolioStatus.js`; a WINNER is 12m revenue NET OF RETURNS > £1,500 (net since 2026-09-26 — "100 sold and 90 returned is a mess, not a winner") and must be IN SEASON — an out-of-season earner is HARVEST until its season returns, so an all-year seller needs season 'Any'; an Amazon SKU takes its style's status; the FIRST and DEFAULT tab on the Repricing screen, Status | Segment | Campaign — bare `/segments` opens it, the segment view is `?by=segment`). **A status list is ONE unsplit list, OUT OF STOCK INCLUDED** (`routes/pricing-status-list.js` styles / `routes/amz-status-list.js` SKUs: every style/SKU with the status, no Selling/Stuck bars — owner: "not much point in splitting" — and 0-stock rows listed so prices can be set ahead of stock arriving, "don't want to leave them at old clearance price"). The segment/campaign lists still require stock. **Each style also carries a stamped LEAD CHANNEL** (`skusummary.portfolio_channel` SHP | AMZ | BOTH: ≥80% of 12m revenue on one channel, else BOTH; unsold = BOTH if on Amazon else SHP; NULL reads as BOTH) — the status stays one all-channel tag, but a channel's status lists (and Repricing tiles) show only its own + BOTH styles (`utils/portfolioStatus.js → channelFilterSql`, the one predicate). Why: 23 of 73 winners were Amazon winners earning ~2.5% on Shopify. **The Winners screen was RETIRED 2026-09-25 and folded into the Repricing Status tab** ("the same screen for two different points of view" — repricing, and analysing winners; the dashboard Winners card and `/analytics/winners` redirect to `/segments`). That tab shows a Shopify row and an Amazon row of five tiles — each tile's number is STYLES (Amazon too), its due line is styles due (an Amazon style is due if ANY size is) — with **no cross-channel total anywhere** (BOTH styles overlap; owner wants the channels split cleanly). A tier toggle (£1,500/2,500/5,000/10,000, `?bar=`) is a LOOKUP WITHIN THE TAG: it narrows only the WINNERS tiles (stamped all-channel revenue) and dims the other eight — a hidden winner has no known other status, and storing a fallback status was rejected. Winners by brand sit inside each WINNERS tile. There is NO status trend: it was removed 2026-09-25 (graph + `portfolio_status_snapshot`), because once a winner had to be in season the line charted the calendar and season edits, not the business. Seasons are reviewed in bulk on **Back Office → Seasons** (`/seasons`). Status **replaced Top earners**, which was removed entirely on 2026-09-24 (tab, cards, route, grouping — git history if ever wanted). Same styles, bars, drill and writes — only the slice differs. Then a **WINNERS | LOSERS** switch:

> **UI names (owner, 2026-09-23):** on screen, WINNERS | LOSERS are **Selling | Stuck** (| Both), and the `/segments` screen is **Repricing** (nav, tile, title, back links). Labels only — routes, `?mode=winners|losers|all`, API/route names and code identifiers keep the old words. "Winners" in the UI now means ONLY the portfolio WINNERS status (its tiles on Repricing's Status tab); don't reintroduce it for the Selling | Stuck lists.
...
- **Size = the code's suffix after the last `-`** — SQL `SUBSTRING(code FROM '[^-]*$')`. **Not `RIGHT(code,2)`**: ~10% of live codes are half sizes (`…-37.5` → `.5`, so 37.5/38.5/40.5 merged into one size) and some brands use UK sizes (`…-8` → `-8`). Sort numerically with `~ '^[0-9]+([.][0-9]+)?$'` → `::numeric` (write `[.]`, not `\.`, inside a JS template string). Fixed app-wide 2026-09-24.
