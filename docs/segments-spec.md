...
- ~~**Apply park default** — W-A1 parks with `AMZ_DEFAULT_REVIEW_DAYS = 14` when the client sends no `reviewDays`.~~ **RESOLVED (2026-07-12):** owner asked to match Shopify exactly — the review is now **OPTIONAL with a default of None** (no park). `AmzPriceSetter` shows the SAME pills as Shopify — `None / 3 / 5 / 7 / 10 / 14 / 30 / 90` days (None default), and `amz-apply` treats omitted/blank/null `reviewDays` as "leave `next_amz_price_review` untouched" (the `AMZ_DEFAULT_REVIEW_DAYS` const was removed). The drill's success notice names the outcome ("parked … / stays in the list").
