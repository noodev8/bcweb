...
- **Later edit stages (NOT started):** Title (+generate), Price/Cost/RRP/Tax (via pricing W1 route — shopifychange + price_change_log), Sizes (skumap add/edit/reorder). Open Q: changing colour/producttype/title feeds the Shopify listing but currently doesn't set shopifychange — decide if/when catalogue edits should flag a sync.
