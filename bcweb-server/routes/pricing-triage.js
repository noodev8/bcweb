...
Key domain rules baked into the SQL (S2, CLAUDE.md) — do not change without re-reading CLAUDE.md:
  ...
  - Drop "parked" styles: a future next_shopify_price_review hides a style until the cooldown passes — unless ?parked=include, which
    keeps them in and flags each one (parked:true + its next_review date) so the page can show them as "pending review".
...
Request Query Params:
  segment  (string)            - the segment to shortlist within. Give exactly one of segment / campaign / status.
  ...
  status   (string)            - a portfolio status. Accepted (utils/pricingGroup.js), but the Repricing screen reads a status from
                                 pricing-status-list, which is one unsplit list — not this route's bar.
  ...
  parked   (string, optional)  - 'include' = also return PARKED styles (future next_shopify_price_review), each flagged parked:true.
                                 ...
                                 "show pending review" toggle without a second route; the WINNERS bar itself is unchanged.
...
Success Response:
{
  ...
  "rows": [
    { "rank": 1, "groupid": "ABC123", "title": "Arizona Birko-Flor", "units": 25, "stock": 8, "price": 36.95, "match_amazon": false,
      "next_review": "2026-10-02", "parked": false },   // next_review YYYY-MM-DD or null; parked = next_review is in the future
